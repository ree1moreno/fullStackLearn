...
function addCrewMember(
  ...
  if (spaceship.crew.length >= spaceship.crewLimit) {
    ...
    alert(`${member} adicionado a tripulação da ${spaceship.name}`);
  }
}
...
function firstMenuOption() {
  ...
  const pilot = prompt(`Qual é o nome do piloto a ser registrado?`);
...
function thirdMenuOption() {
  ...
  if (spaceship) {
    ...
    if (confirmation) sendInMission(spaceship);
  }
}
